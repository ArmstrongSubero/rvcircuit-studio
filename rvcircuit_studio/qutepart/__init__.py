"""qutepart --- Code editor component for PySide6
=========================================================
"""

# Enable short-form enum access (Qt.Key.Key_X instead of Qt.Key.Key_X)
# This maintains compatibility with the existing codebase without
# rewriting hundreds of enum references.
import os
os.environ.setdefault("QT_API", "pyside6")

try:
    from PySide6.QtCore import __version__ as _pyside_version
    _major, _minor = (int(x) for x in _pyside_version.split(".")[:2])
    if _major >= 6 and _minor >= 4:
        # PySide6 6.4+ supports short-form enums natively
        pass
except Exception:
    pass
...
from PySide6.QtCore import QKeyCombination, QRect, Qt, Signal
from PySide6.QtWidgets import QApplication, QDialog, QPlainTextEdit, QTextEdit, QWidget
...
class Qutepart(QPlainTextEdit):
    ...
    **Public methods**
    ...
    def __init__(self,
        ...
        self.setAttribute(Qt.WidgetAttribute.WA_KeyCompression, False)  # vim can't process compressed keys
    ...
    def _setSolidEdgeGeometry(self):
        ...
        if self._lineLengthEdge is not None:
            ...
            # gives 4
            x = self.fontMetrics().horizontalAdvance('9' * self._lineLengthEdge) + \
    ...
    def keyPressEvent(self, event):
        ...
        if event.matches(QKeySequence.InsertParagraphSeparator):
            ...
            for action in self.actions():
                seq = action.shortcut()
                if seq.count() == 1 and seq[0] == QKeyCombination(event.modifiers(), Qt.Key(event.key())):
                    action.trigger()
                    break
            else:
                if event.text() and event.modifiers() == Qt.KeyboardModifier.AltModifier:
    ...
    def keyReleaseEvent(self, event):
        if self._lastKeyPressProcessedByParent and self._completer is not None:
            ...
            text = event.text()
            textTyped = (text and \
                         event.modifiers() in (Qt.KeyboardModifier.NoModifier, Qt.KeyboardModifier.ShiftModifier)) and \
    ...
    def _onShortcutHome(self, select):
        ...
        if cursor.positionInBlock() == indent:
            ...
            # block.
            cursor.movePosition(QTextCursor.MoveOperation.StartOfBlock, anchor)
            ...
            cursor.movePosition(QTextCursor.MoveOperation.StartOfLine, anchor)
