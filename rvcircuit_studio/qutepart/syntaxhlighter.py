...
from PySide6.QtGui import QTextBlockUserData, QTextLayout
...
try:
    ...
    # Fallback: assume object is always valid
    def _isValid(obj):
...
class SyntaxHighlighter(QObject):
    ...
    def _highlighBlocks(self, fromBlock, atLeastUntilBlock, timeout):
        ...
        # successfully finished, reset pending tasks
        self._pendingBlockNumber = None
        self._pendingAtLeastUntilBlockNumber = None
        ...
        """
        if not hasattr(self, "_textEdit") or self._textEdit is None:
            return

        if not _isValid(self._textEdit):
            return

        if self._textEdit and self._textEdit.document():
