...
for charCode in range(ord('a'), ord('z') + 1):
    ...
    setattr(thismodule, '_' + shortName.upper(), Qt.KeyboardModifier.ShiftModifier | qtCode)

_0 = Qt.Key.Key_0
_Dollar = Qt.KeyboardModifier.ShiftModifier | Qt.Key.Key_Dollar
_Percent = Qt.KeyboardModifier.ShiftModifier | Qt.Key.Key_Percent
_Caret = Qt.KeyboardModifier.ShiftModifier | Qt.Key.Key_AsciiCircum
_Esc = Qt.Key.Key_Escape
_Insert = Qt.Key.Key_Insert
_Down = Qt.Key.Key_Down
_Up = Qt.Key.Key_Up
_Left = Qt.Key.Key_Left
_Right = Qt.Key.Key_Right
_Space = Qt.Key.Key_Space
_BackSpace = Qt.Key.Key_Backspace
_Equal = Qt.Key.Key_Equal
_Less = Qt.KeyboardModifier.ShiftModifier | Qt.Key.Key_Less
_Greater = Qt.KeyboardModifier.ShiftModifier | Qt.Key.Key_Greater
_Home = Qt.Key.Key_Home
_End = Qt.Key.Key_End
_PageDown = Qt.Key.Key_PageDown
_PageUp = Qt.Key.Key_PageUp
_Period = Qt.Key.Key_Period
_Enter = Qt.Key.Key_Enter
_Return = Qt.Key.Key_Return


def code(ev):
    modifiers = ev.modifiers()
    modifiers &= ~Qt.KeyboardModifier.KeypadModifier  # ignore keypad modifier to handle both main and numpad numbers
...
def isChar(ev):
    ...
    if ev.modifiers() not in (Qt.KeyboardModifier.ShiftModifier, Qt.KeyboardModifier.KeypadModifier, Qt.KeyboardModifier.NoModifier):
        return False
    ...
    if text == ' ' and ev.modifiers() == Qt.KeyboardModifier.ShiftModifier:
...
class Vim(QObject):
    ...
    def keyPressEvent(self, ev):
        ...
        """
        if ev.key() in (Qt.Key.Key_Shift, Qt.Key.Key_Control,
    ...
    def extraSelections(self):
        ...
        selection.cursor.movePosition(QTextCursor.NextCharacter, QTextCursor.MoveMode.KeepAnchor)
...
class BaseCommandMode(Mode):
    ...
    def _moveCursor(self, motion, count, searchChar=None, select=False):
        ...
        moveMode = QTextCursor.MoveMode.KeepAnchor if select else QTextCursor.MoveMode.MoveAnchor

        moveOperation = {_b: QTextCursor.WordLeft,
                         _j: QTextCursor.MoveOperation.Down,
                         _Down: QTextCursor.MoveOperation.Down,
                         _k: QTextCursor.MoveOperation.Up,
                         _Up: QTextCursor.MoveOperation.Up,
                         _h: QTextCursor.MoveOperation.Left,
                         _Left: QTextCursor.MoveOperation.Left,
                         _BackSpace: QTextCursor.MoveOperation.Left,
                         _l: QTextCursor.MoveOperation.Right,
                         _Right: QTextCursor.MoveOperation.Right,
                         _Space: QTextCursor.MoveOperation.Right,
                         _w: QTextCursor.WordRight,
                         _Dollar: QTextCursor.MoveOperation.EndOfBlock,
                         _End: QTextCursor.MoveOperation.EndOfBlock,
                         _0: QTextCursor.MoveOperation.StartOfBlock,
                         _Home: QTextCursor.MoveOperation.StartOfBlock,
                         'gg': QTextCursor.MoveOperation.Start,
                         _G: QTextCursor.MoveOperation.End
                        }


        if motion == _G:
            if count == 0:  # default - go to the end
                cursor.movePosition(QTextCursor.MoveOperation.End, moveMode)
            ...
            for _ in range(effectiveCount):
                ...
                if motion == _e:
                    cursor.movePosition(QTextCursor.MoveOperation.EndOfWord, moveMode)
                else:
                    text = cursor.block().text()
                    pos = cursor.positionInBlock()
                    for char in text[pos:]:
            ...
            if motion in (_f, _t):
                ...
                stepForward = QTextCursor.MoveOperation.Right
                stepBack = QTextCursor.MoveOperation.Left
            else:
                iterator = self._iterateDocumentCharsBackward(cursor.block(), cursor.columnNumber())
                stepForward = QTextCursor.MoveOperation.Left
                stepBack = QTextCursor.MoveOperation.Right
            ...
            direction = QTextCursor.MoveOperation.Down if motion == _PageDown else QTextCursor.MoveOperation.Up
            ...
            if cursor.block().next().isValid():  # not the last line
                for _ in range(effectiveCount):
                    cursor.movePosition(QTextCursor.MoveOperation.NextBlock, moveMode)
                    self.moveToFirstNonSpace(cursor, moveMode)
        else:
    ...
    def _expandSelection(self):
        ...
        if pos >= anchor:
            anchorSide = QTextCursor.MoveOperation.StartOfBlock
            cursorSide = QTextCursor.MoveOperation.EndOfBlock
        else:
            anchorSide = QTextCursor.MoveOperation.EndOfBlock
            cursorSide = QTextCursor.MoveOperation.StartOfBlock


        cursor.setPosition(anchor)
        cursor.movePosition(anchorSide)
        cursor.setPosition(pos, QTextCursor.MoveMode.KeepAnchor)
        cursor.movePosition(cursorSide, QTextCursor.MoveMode.KeepAnchor)
...
class BaseVisual(BaseCommandMode):
    ...
    def _selectRangeForRepeat(self, repeatLineCount):
        ...
        cursor = self._qpart.textCursor()
        cursor.movePosition(QTextCursor.MoveOperation.EndOfBlock, QTextCursor.MoveMode.KeepAnchor)  # expand until the end of line
        self._qpart.setTextCursor(cursor)
    ...
    def cmdJoinLines(self, cmd, repeatLineCount=None):
        ...
        with self._qpart:
            for _ in range(count):
                ...
                cursor.movePosition(QTextCursor.NextCharacter, QTextCursor.MoveMode.KeepAnchor)
                self.moveToFirstNonSpace(cursor, QTextCursor.MoveMode.KeepAnchor)
...
class Normal(BaseCommandMode):
    ...
    def cmdDelete(self, cmd, count):
        ...
        """
        cursor = self._qpart.textCursor()
        direction = QTextCursor.MoveOperation.Left if cmd == _X else QTextCursor.MoveOperation.Right
        for _ in range(count):
